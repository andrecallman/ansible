...
<?php
if(isset($_GET['customRestartAllPollers'])){
    $AllPollers = array();
    $DBRESULT = $pearDBO->query("SELECT SQL_CALC_FOUND_ROWS DISTINCT id, name FROM centreon.nagios_server WHERE name LIKE '%%' AND ns_activate=1 ORDER BY name");
    $opt=null;
    while ($info = $DBRESULT->fetchRow()) {
        $AllPollers[] = $info;
        $opt.='<option value="'.$info['id'].'" selected>'.$info['name'].'</option>';
    }
    $DBRESULT->free();
    ?>
    <script>
        var nhost = document.getElementById('nhost');
        nhost.innerHTML = '<?php echo $opt; ?>';
        var nmove = document.getElementById('nmove');
        nmove.checked = true;
        var nrestart = document.getElementById('nrestart');
        nrestart.checked = true;
        var nrestart_mode = document.getElementById('nrestart_mode');
        nrestart_mode.value='2';
    </script>
    <?php
}
?>
